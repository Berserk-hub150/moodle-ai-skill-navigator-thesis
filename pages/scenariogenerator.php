...
if ($generate === 1) {
    ...
    if ($sourcemode === 'manual') {
        ...
        if (!empty($results)) {
            ...
            foreach ($results as $ragresult) {
                $ragsources[$ragresult->title . ' ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â chunk ' . (((int) $ragresult->chunkindex) + 1)] = $ragresult->similarity;
            }
    ...
    if (trim($result) === '' && $warning === '') {
        $warning = 'La generazione ÃƒÆ’Ã‚Â¨ partita, ma il servizio AI ha restituito una risposta vuota.';
    }
}
